...
df = pd.DataFrame(columns=['cafe_name', 'naver_map_url'])
...
for i in range (page_num):
    ...
    span_elements = scroll_element.find_elements(By.CLASS_NAME, "place_bluelink.TYaxT")
    time.sleep(2)
    ...
    for j in range(span_text_len):
        ...
        element_xpath = f'//*[@id="_pcmap_list_scroll_container"]/ul/li[{j + 1}]/div'
        ...
        place_element_a = place_element.find_element(By.CLASS_NAME, "tzwk0")  # a 태그 ID에 맞게 변경
    ...
    print(df)
    df.to_csv('./cafename.csv', index=False, encoding='utf-8-sig')
